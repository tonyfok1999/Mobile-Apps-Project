...
export class ChatroomController {
  ...
  @Post('/orderChatroom')
  async createChatroom(@Query() query: { orderId: number; userId: number; workerId: number }) {
    const orderId = query.orderId;
    const userId = query.userId;
    const workerId = query.workerId;

    console.log( { orderId: orderId, userId: userId, workerId: workerId })

    if (typeof query.orderId !== 'number' || typeof query.userId !== 'number' || typeof query.workerId !== 'number') {
    ...
    try {
      ...
      if (result.rowCount > 0) {
        throw new HttpException('the chatroom already exist', HttpStatus.FORBIDDEN);
      } else {
